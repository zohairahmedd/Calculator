...
def button_add(): # function button_add with no argument
    ...
    f_num = int(first_number) # assigns integer value of first_number to f_num (global variable)
    e.delete(0, END) # deletes all content fron entry widget
...
def button_equal(): # function button_equal with no argument
    ...
    if math == "addition":
        e.insert(0, f_num + int(second_number)) # adds first and second number if math is addition
    if math == "subtraction":
        e.insert(0, f_num - int(second_number)) # subtracts first and second number if math is subtraction
    if math == "multiplication":
        e.insert(0, f_num * int(second_number)) # multiplies first and second number if math is multiplication
    if math == "division":
        e.insert(0, f_num / int(second_number)) # divides first and second number if math is division
...
# positions each button at a specific location in the grid defined by row and column parameters | notice how nothing is in the same row & column
